-- ============================================================
-- ClinicQ Step 4 — Dynamic & Custom Form Fields
-- Run AFTER step1, step2, step3 schemas
-- ============================================================

-- ─────────────────────────────────────────────────────────
-- PART 1: Extend hospital_settings with field toggles
-- ─────────────────────────────────────────────────────────
...
-- custom_fields JSONB schema (array of field objects):
-- [
--   {
--     "id":       "field_abc123",   -- unique per hospital
--     "label":    "Blood Group",    -- display label
--     "type":     "text",           -- "text" | "number" | "dropdown"
--     "required": false,            -- whether field is mandatory
--     "options":  []                -- only for type=dropdown
--   }
-- ]

COMMENT ON COLUMN public.hospital_settings.enable_age IS
  'Show/hide the Age field on the patient check-in form';

COMMENT ON COLUMN public.hospital_settings.enable_reason IS
  'Show/hide the Reason for Visit dropdown on the check-in form';

COMMENT ON COLUMN public.hospital_settings.custom_fields IS
  'JSON array of custom field definitions added by the hospital';

-- ─────────────────────────────────────────────────────────
-- PART 2: Extend queue_entries with custom_data
-- ─────────────────────────────────────────────────────────

ALTER TABLE public.queue_entries
  ADD COLUMN IF NOT EXISTS custom_data JSONB DEFAULT '{}'::JSONB;

-- custom_data JSONB schema (keyed by field id):
-- {
--   "field_abc123": "B+",
--   "field_def456": "42"
-- }

COMMENT ON COLUMN public.queue_entries.custom_data IS
  'Stores values for hospital-defined custom check-in fields';

-- Index for querying custom_data fields
CREATE INDEX IF NOT EXISTS idx_queue_entries_custom_data
  ON public.queue_entries USING gin (custom_data);

-- ============================================================
-- RPC: get_hospital_full (UPDATED)
-- Now includes enable_age, enable_reason, custom_fields
-- ============================================================
...
-- ============================================================
-- RPC: create_queue_entry (UPDATED)
-- Accepts p_custom_data JSONB for custom field values
-- ============================================================
...
DECLARE
  ...
  v_lock_key       BIGINT;
BEGIN
  -- Advisory lock: serialize token generation per hospital per day
  ...
  -- Upsert daily state and increment token
  ...
  -- Insert queue entry with custom_data
...
-- ============================================================
-- RPC: update_hospital_settings (UPDATED)
-- Now includes enable_age, enable_reason, custom_fields
-- ============================================================
...
BEGIN
  ...
  IF p_token_limit IS NOT NULL AND
     (p_token_limit < 1 OR p_token_limit > 500) THEN
  ...
  -- Validate custom_fields is a JSON array
  IF p_custom_fields IS NOT NULL AND
     jsonb_typeof(p_custom_fields) != 'array' THEN
...
-- ============================================================
-- Update get_queue_today to include custom_data in entries
-- ============================================================
